...
def loadParams(files, beamlims):
        ...
        dataStore = n5esmrStorage(params)
        numAccepted = 0
        for file in files:
            #applying filters
        ...
        if numAccepted > 0:
            dataStore.merge()
            dataStore.remove_outer_beams(beamlims[0], beamlims[1])
            ...
            data['incidence_angle'] = np.ones(data['tcw'].shape)*spos2ia[None, beamlims[0]:beamlims[1]] #Why do we use IA when already corrected for?
            
            mt2m = np.copy(data['t2m'])
            mt2m[(mt2m < 273.15)] = 273.15
            data['mt2m'] = mt2m
            return data
