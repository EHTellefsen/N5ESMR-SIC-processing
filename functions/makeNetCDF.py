...
def makeNetCDF(data_vars, date, hemisphere, area_def, save_dir):
    ...
    ncfile_n = ("ESACCI-SEAICE-L3C-SICONC-NIMBUS5_ESMR-EASE2_%sH-%s%s%s-fv1.1.nc" % (hemisphere, year, month, day))
...
##    dNN.time_bnds.encoding['units'] = "days since "+aar+'-'+maaned+'-'+dag+'T12:00:00.000000000'
    ...
    dNN.status_flag.attrs['valid_min'] = 0,
    dNN.status_flag.attrs['valid_max'] = 32767,
    dNN.status_flag.attrs['grid_mapping'] = 'Lambert_Azimuthal_Grid',
    dNN.status_flag.attrs['flag_masks'] = np.array([1,2,4,8,16,32,64,128],dtype='int32')
    ...
    #save to netcdf
    dNN.to_netcdf(path=ncfile_path, mode ='w',format='NETCDF4_CLASSIC',encoding = {'time_bnds': {'_FillValue': None},
                                                                                   "ice_conc": {"zlib": True, "dtype": "float32"},
                                                                                   "raw_ice_conc_values": {"zlib": True, "dtype": "float32"},
                                                                                   "total_standard_error": {"zlib": True, "dtype": "float32"},
                                                                                   "smearing_standard_error": {"zlib": True, "dtype": "float32"},
                                                                                   "algorithm_standard_error": {"zlib": True, "dtype": "float32"},
                                                                                   "Tb_corr": {"zlib": True, "dtype": "float32"},
...
# %% LDTP
def LDTP_NetCDF(data_vars, date, hemisphere, area_def, save_dir):
    ...
    # specifying outfile name and path
    ncfile_n = ("ESACCI-SEAICE-L3C-SICONC-NIMBUS5_ESMR-EASE2_LDTPcorrected_%sH-%s%s%s-fv1.1.nc" % (hemisphere, year, month, day))
    ncfile_path = os.path.join(save_dir, ncfile_n)
    ...
    attrs=dict(
        description="Weather related data.",
        title = 'Sea Ice Concentration Climate Data Record from ESA Climate Change Initiative',
        summary = 'This climate data record of sea ice concentration is obtained from passive microwave satellite data over the polar regions (ESMR). The processing chain features: 1) dynamic tuning of tie-points and algorithms,  2) correction of atmospheric noise using a Radiative Transfer Model,  3) computation of per-pixel uncertainties, 4) one channel sea ice concentration algorithm, 5) correction through local tiepoints utilizing general stability of Brightness temperature signal. This dataset was generated by the ESA Climate Change Initiative.',
        opiccategory = 'Oceans Climatology Meteorology Atmosphere',
        keywords = 'Earth Science > Cryosphere > Sea Ice > Sea Ice Concentration\nEarth Science > Oceans > Sea Ice > Sea Ice Concentration\nEarth Science > Climate Indicators > Cryospheric Indicators > Sea Ice Concentration',
        keywords_vocabulary = 'GCMD Science Keywords',
        geospatial_lat_min = np.min(latss),
...
##    dNN.time_bnds.encoding['units'] = "days since "+aar+'-'+maaned+'-'+dag+'T12:00:00.000000000'
    ...
    dNN.to_netcdf(path=ncfile_path, mode ='w',format='NETCDF4_CLASSIC',encoding = {'time_bnds': {'_FillValue': None},
                                                                                   ...
                                                                                   "Tb_corr": {"zlib": True, "dtype": "float32"},
                                                                                   "Tb": {"zlib": True, "dtype": "float32"}, 
                                                                                   "status_flag": {"zlib": True, "dtype": 'int32', "_FillValue": -1}
                                                                                   },
                                                                                   unlimited_dims={'time':True})
